    ...
    <div className="app-layout">
      <nav className="app-nav">
        ...
        <div className={`app-nav-links${menuOpen ? ' open' : ''}`}>
          {NAV_LINKS.map(link => (
            <a key={link} className="app-nav-link" href={`#${link}`} onClick={() => setMenuOpen(false)}>{link}</a>
          ))}
        </div>
      </nav>
